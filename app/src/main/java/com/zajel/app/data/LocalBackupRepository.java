...
public final class LocalBackupRepository {
    ...
    public JSONObject inspect(Uri source) throws Exception {
        ...
        private JSONArray collectMessageRecords() throws Exception {
        ...
        for (File file : files) {
            ...
            if (!file.getName().toLowerCase().endsWith(".json")
                    && !file.getName().toLowerCase().contains("message")) {
                continue;
            }
        ...
        if (records.length() == 0) {
            records.put(
                new JSONObject()
                    .put("type", "message")
                    .put("source", "none")
                    .put("note", "No persisted message records were found in app storage.")
            );
        }
  ...
  private JSONArray collectMediaRecords() throws Exception {
        JSONArray records = new JSONArray();
        ...
        if (records.length() == 0) {
            records.put(
                new JSONObject()
                    .put("type", "media")
                    .put("source", "none")
                    .put("note", "No cached media files were found.")
            );
        }
    ...
    private JSONArray collectFileRecords() throws Exception {
        JSONArray records = new JSONArray();
        ...
        if (records.length() == 0) {
            records.put(
                new JSONObject()
                    .put("type", "file")
                    .put("source", "none")
                    .put("note", "No imported files were found in the local file store.")
            );
        }
    ...
    private void appendFiles(JSONArray records, File root, String type) throws Exception {
